...
public interface JNAKernel32 extends StdCallLibrary {
	 ...
	 * lpMessageCount, ref ulong lpReadTimeout) library "kernel32.dll" function
	 * boolean SetMailslotInfo (ulong hMailslot, ulong lReadTimeout) library
	 * "kernel32.dll" function ulong CreateFile (string lpFileName, ulong
	 * dwDesiredAccess, ulong dwShareMode, ulong lpSecurityAttributes, ulong
	 * dwCreationDisposition, ulong dwFlagsAndAttributes, ulong hTemplateFile)
	 * library "kernel32.dll" alias for "CreateFileW" function boolean ReadFile
	 * (ulong hFile, ref string lpBuffer, ulong nNumberOfBytesToRead, ref ulong
	 * lpNumberOfBytesRead, ulong lpOverlapped) library "kernel32.dll" function
	 * boolean WriteFile (ulong hFile, ref string lpBuffer, ulong
	 * nNumberOfBytesToWrite, ref ulong lpNumberOfBytesWritten, ulong
